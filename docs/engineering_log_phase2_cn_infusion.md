...
| Alpha 训练无法“环扣”到多专家（news/chartist/system2 等），信用分配缺失 | `step_experiences.jsonl` 缺少 router/专家输出/system2/chartist/风控的 attribution，无法按专家切分训练集 | 新增 `joint_step_experiences.jsonl`：记录 `decision_trace`（router + selected_expert + news/chartist/system2 + risk）并支持一键 `source=joint_step` 训练多个专家（DPO + 可选 VLM SFT） | `src/rl/online_learning.py`、`src/trading/strategy.py`、`scripts/generate_alpha_joint_datasets.py`、`scripts/alpha_joint_evolution.py` | live 运行后 `data/rl_experiences/joint_step_experiences.jsonl` 行数增长；调用 `/api/v1/evolution/alpha/train/start?source=joint_step` 产生多个输出目录 |
| 训练后不知道是否已加载新模型（热切换不可观测） | 训练产物只是落盘目录，live 侧没有统一的 active pointer 回显 | 写指针文件 `active_trading_models.json` / `active_chartist_adapter.json`；`/api/v1/live/reload_models` 按指针热切换并 reload；`/api/v1/live/status` 回显当前 `moe_*` 与 `chartist_adapter` 路径 | `src/api/main.py`、`data/finetune/evolution/active_trading_models.json`、`data/finetune/evolution/active_chartist_adapter.json` | 训练完成后调用 reload，再调用 status 可看到当前 active adapter 路径与切换结果 |
...
### 3.4.1 Multi-Agent「人海战术」+ RL→Alpha Evolution 数据闭环（2026-01-06）



目标：让所有 Agent 并发参与决策并写入可训练数据；RL 采集的数据能直接进入 Alpha Evolution，训练产物自动切换到 live，无需手改 YAML/重启进程。
...
- **Live 强制启用真实 Gatekeeper/Planner**：

  - Gatekeeper 不再随机，通过 `models/rl_gatekeeper_v*.pt` 推理 `q_allow` 决策。

  - Planner 支持 `models/planner_sft_v1.pt` 的 SFT policy（可由配置打开）。

- **轨迹/收益闭环**：

  - `all_agents_mode` 下 scalper/analyst 都会写入 `trajectory`（各自一条 trace id）。

  - 平仓时按持仓绑定的 `trace_ids` 逐条写入 `outcome`（realized_pnl），使 `scripts/generate_alpha_dataset.py` 可生成 `dpo_alpha_nightly.jsonl`。

- **训练产物自动切换（Trading MoE）**：

  - Alpha Evolution 训练成功后写 `data/finetune/evolution/active_trading_models.json` 指向最新 `moe_analyst` adapter。

  - Live 启动时优先读取该指针文件覆盖 `trading.moe_scalper/moe_analyst`。

  - `POST /api/v1/live/restart` 支持热切换：读取指针文件→覆盖 adapter→重新 `_load_models()`，并在 dashboard terminal 输出 `[HotSwap] ...`。
...
- `configs/secretary.yaml`：新增 `trading.*`（all_agents_mode / committee_policy / planner_policy / gatekeeper_model / moe_* 等）。

- `src/trading/strategy.py`：

  - `all_agents_mode` 双专家并行决策 + committee policy。

  - 强制启用 Gatekeeper/Planner（真实模型）。

  - 生成 `trace_ids` 写入 signal。

- `src/trading/broker.py`：

  - BUY 绑定 `trace_ids` 到持仓；SELL 平仓写入 `outcome` 到 trajectories。

  - fill payload 透传 `expert/analysis/trace_ids`。

- `scripts/run_live_paper_trading.py`：启动时读取 `active_trading_models.json` 并输出 `[Models] ...`。

- `src/api/main.py`：

  - Alpha train watcher 成功后写 `active_trading_models.json`。

  - `/api/v1/live/restart` 加入 MoE 热切换逻辑。
...
1. 启动 live，terminal 出现 `[Models] scalper=... | analyst=...`。

2. 触发至少一轮 BUY+SELL（闭环成交）。

3. `data/evolution/trajectories/YYYYMMDD.jsonl` 同时出现：

   - `type=trajectory, agent_id=scalper/analyst`

   - `type=outcome, ref_id=<trace_id>, outcome=<realized_pnl>`

4. 点 Alpha Evolution (train)：成功后生成 `active_trading_models.json`。

5. 点 Live Restart：terminal 出现 `[HotSwap] scalper=... | analyst=...`，并使用新 analyst adapter 继续推理。

## 2026-01: Architecture A2 Migration (Shared Base + MoE Hot-Swap)

**Status**: Completed

**目标**: 移除独立的 Secretary LLM 进程，将对话/秘书能力迁移至交易底座 (Qwen 2.5 7B)，通过 LoRA Hot-Swap 实现显存减半与架构统一。

### 1. 核心变更
- **模型统一**: 废弃 `Qwen/Qwen3-8B` (Secretary)，统一使用 `Qwen/Qwen2.5-7B-Instruct` (Trading Base)。
- **MoE 扩展**: 在 Scalper/Analyst 基础上新增 `secretary` Adapter。
- **推理分时复用**:
  - 交易时段 (`on_bar`): 使用 `scalper` 或 `analyst`。
  - 对话时段 (`chat`/`mari_voice`): 瞬间切换 `secretary` Adapter，生成后切回。
- **显存优化**: 移除独立 Python 进程与模型加载，显存占用降低约 5GB。

### 2. 组件改造
- **Strategy**: 新增 `generic_inference` 支持指定 Adapter 推理；引入 `threading.Lock` 保证 API/EventLoop 线程安全。
- **API**: `/api/v1/chat` 直接调用 `strategy.generate_reply`；注入 `agent_logs` 到系统提示词以增强上下文感知。
- **LiveRunner**: `MariVoice` 语音解说不再调用 HTTP API，而是直接调用注入的 `strategy` 实例进行推理。
- **Training**: `nightly_evolution.py` 适配 A2 架构，读取 `trading.moe_secretary` 配置，并支持新闻链接 (`shared_urls`) 的上下文学习。

### 3. 验证结果
- **启动**: 单进程加载 Qwen 7B + 3 LoRAs (Scalper, Analyst, Secretary) 成功。
- **对话**: 聊天接口响应正常，能感知系统状态 (Logs)。
- **语音**: Mari 语音解说通过共享底座生成成功。
- **训练**: Planner OLRL (Nightly Evolution) 数据管道打通。

**下一步**:
- 观察实盘运行中的 GIL/Lock 争抢情况（虽然推理是毫秒级，但在极端高频行情下需关注延迟）。
- 持续收集 Planner OLRL 数据，优化 Secretary LoRA 的指令遵循能力。
