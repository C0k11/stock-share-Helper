...
This repo never contained a Tableau workbook — only the written spec. The
working principle: **spec unchanged, engine swapped, results reconciled**.
...
row count (500), `DISTINCTCOUNTNOBLANK(dim_symbol[symbol])` (35), and a canary
...
Result on 2026-07-28 (post-cleanup): **66 / 67 PASS, 1 SKIPPED by design**
...
key and every symbol key is 0-null **except `fact_news.symbol` — 500 of 5,581
rows are null** (news not attributable to one ticker). Once `symbol_to_news`
went live, those 500 rows matched nothing in `dim_symbol` and Power BI
...
which the `rowcount_dim_symbol` check now expects). The 500 news rows keep
